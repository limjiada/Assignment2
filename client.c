...
void createFile(int sockfd){	//Creating files on the client site
	...
	if(filename != NULL){
		strcat(dir, filename);
		printf("File location is at : %s", dir);
		//Create file
		FILE *fp;
		fp = fopen(dir, "w+");
		if(fp == NULL){
		  printf("\nERROR: File is failed to create");
		  perror("fopen");	
		  exit(0);	
		}
...
void downloadFile(int sockfd){	//Client download file from Server
	printf("Downloading a file from the server... ");
	...
	printf("Enter the file that you wish to download: ");
	char selectFile[256];
